...
if ($request.url.indexOf(url1) != -1) {
    ...
    resBody = resBody.replace('"is_share":0', '"is_share":1');
    resBody = resBody.replace('"is_show":false', '"is_show":true');
}
else if ($request.url.indexOf(url2) != -1) {
    resBody = resBody.replace(/"status":[^,]+/g, '"status":1');
    resBody = resBody.replace(/"isSubscribed":true/g, '"isSubscribed":false');
    resBody = resBody.replace('"isShare":0', '"isShare":1');
    resBody = resBody.replace('"isShow":false', '"isShow":true');
}
